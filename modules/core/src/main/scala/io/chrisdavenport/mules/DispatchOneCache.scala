...
final class DispatchOneCache[F[_], K, V] private[DispatchOneCache] (
...
)(implicit val F: Concurrent[F], val C: Clock[F]) extends LifetimeCache[F, K, V]{
   ...
   **/
  override final def insertWithTimeout(optionTimeout: Option[TimeSpec])(k: K, v: V): F[Unit] = for {
